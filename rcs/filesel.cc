...
1.3
date	97.07.14.04.21.44;	author dlorre;	state Exp;
branches;
next	1.2;

1.2
date	96.08.28.20.02.41;	author dlorre;	state Exp;
branches;
next	1.1;
...
@
text
@// $Id: filesel.cc 1.2 1996/08/28 20:02:41 dlorre Exp dlorre $
#include "filesel.h"
...
1.2
log
@passage gprop sur la border droite de la fenêtre
@
text
@d1 1
a1 1
...
d124 1
a124 1
    if (ws->screen->ViewPort.Modes & HIRES) {
d137 1
a137 1
...
d179 3
a181 3
            addregion(LEFTBOX, TOPDBOX, WIDTHBOX, HEIGHTBOX) ;
            addregion(LEFTBOX, TOPFBOX, WIDTHBOX, HEIGHTBOX) ;
            addregion(LEFTSEL, TOPSEL, WIDTHSEL, HEIGHTSEL) ;
d183 1
a183 1
                addregion(LEFTSEL, TOPSEL+HEIGHTSEL+34, LEFTBOX+WIDTHBOX-LEFTSEL, 14) ;
d195 2
d198 1
a198 1
    Plain = Italic = *s->screen->Font ;
d208 1
a208 1
    FONTHEIGHT = short(s->screen->Font->ta_YSize) ;
d221 2
a223 1
    Style = new TextAttr*[TOT_DISPLAY] ;
d233 1
a233 1
        listetext[i].ITextFont = s->screen->Font ;
d235 3
d241 4
d248 3
d252 1
...
d769 1
a769 1
    if ((diskobj=GetDiskObject(fname)) || (diskobj=GetDefDiskObject(deftype))) {
@
...
d124 12
a135 2
    g->box(LEFTSEL+WIDTHSEL+4, TOPSEL, 16, HEIGHTSEL) ;
    gprop = new eprop(g, WFUNC(&filesel::fprop), 0, TOT_DISPLAY, TOT_DISPLAY, LORIENT_VERT) ;
d140 1
a140 1
            g->fontheight+4) ;
d167 1
