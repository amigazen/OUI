...
1.3
date	97.09.17.08.15.27;	author dlorre;	state Exp;
branches;
next	1.2;

1.2
date	97.07.14.04.19.51;	author dlorre;	state Exp;
branches;
next	1.1;
...
@
text
@// $Id: eroll.cc 1.2 1997/07/14 04:19:51 dlorre Exp dlorre $
...
const double c3PID2  = 3*PID2 ;
const double c5PID4  = 5*PID4 ;
const double c7PID4  = 7*PID4 ;
d161 1
a161 1
    double      angMax ;
d165 1
a165 2
static ULONG __asm dispatchERoll(register __a0 Class *cl,
    register __a2 Object *o, register __a1 Msg msg);
d181 1
a181 1
    else if (!(BOOPSIeroll = MakeClass(NULL, "propgclass", NULL,
d186 3
a188 1
        BOOPSIeroll->cl_Dispatcher.h_Entry = (HOOKFUNC)dispatchERoll;
d201 1
a201 3
ULONG __saveds __asm dispatchERoll(register __a0 Class *cl,
...
d241 2
a242 2
                xp = flt(gpi->gpi_Mouse.X - inst->XRadius) ;
                yp = flt(inst->YRadius - gpi->gpi_Mouse.Y) ;
d245 1
a245 1
                    alpha = PID2 ;
d251 9
a259 6
                ...
                if (alpha <= inst->angMin || alpha >= inst->angMax)
                    inst->Top =  (inst->Total * (inst->angPhase - alpha))
                        / inst->angWidth ;
d303 3
a305 3
            double   alpha, xp, yp ;
...
d393 1
a393 1
void __saveds NotifyRoll(Class *cl, Object *o, ULONG flags, long level, gpInput *gpi)
d397 1
d409 1
a409 1
ULONG __saveds RenderERoll(Class *cl, Gadget *g, gpRender *msg)
d417 1
a427 1
        WaitTOF() ;
d437 1
a437 2
                ...
                RectFill(rp, inst->KLeft-2, inst->KTop-2, inst->KRight+1, inst->KBottom+1) ;
a449 3
                SetAPen(rp, inst->XPens[GFILL_PEN]) ;
                Flood(rp, 1, inst->CLeft, inst->CTop) ;

d464 1
a464 1
void __saveds RollSize(RollINST *inst, Gadget *g)
d468 1
d479 1
d492 2
a493 1
    inst->angPhase = inst->angMin = c5PID4 ;
a495 1
        inst->angMax = inst->angMin ;
a498 1
        inst->angMax = c7PID4 ;
d504 1
a504 1
void __saveds RollLims(RollINST *inst)
d506 1
d520 1
a520 1
        fix( cos(inst->KStart) * (inst->XRadius - (inst->XUnity * 3.0)) + .5) ;
d523 1
a523 1
        fix( sin(inst->KStart) * (inst->YRadius - (inst->YUnity * 3.0)) + .5) ;
@
